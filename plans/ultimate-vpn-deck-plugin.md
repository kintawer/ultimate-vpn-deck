...
---
## Changes to plan while implementing

- **Тесты удалены по прямому запросу пользователя** после первичной реализации: `test_unit.py`, `test_profile_manager.py`, `test_plugin_smoke.py`, `infra/test-smoke.sh` убраны из репозитория; `justfile` (`test`/`test-smoke` рецепты) и README очищены от упоминаний тестов. До удаления все три набора (65+14+24 проверки) проходили зелёным — сама логика (парсеры, sing-box конфиг, RPC) осталась непротестированной автоматически, полагаемся на ручную проверку на Steam Deck.
